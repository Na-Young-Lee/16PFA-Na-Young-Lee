...
class SimpleCashCard:
    ...
    # 메소드 methods : # 객체에 어떤 신호를 전달하는 역활을 한다
    def deposit(self, amount_won):
         ...
         :return:
         """
         print "SimpleCahCard deposit()"  # 함수 호출 표식
         # 입금하면 잔고가 증가한다
    ...
    def withdraw(self, amount_won):
        ...
        :return:
        """
        print "SimpleCahCard withdraw()"  # 함수 호출 표식
        # 출금하면 잔고가 감소한다
        self.balance_won += (-amount_won)

    def ckeck_balance(self):
        """
        잔고조회
        :return:
        """
        print "SimpleCahCard check_balance()"  # 함수 호출 표식
        # 통장 잔고를 반환한다
        return self.balance_won
# SimpleCahCard 클래스 정의 끝